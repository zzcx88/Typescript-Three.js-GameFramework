...
export class CollisionComponent
{
    ...
    public CreateBoundingBox(x: number, y: number, z: number)
    {
        ...
        const color = new THREE.Color().setColorName("Red");
    ...
    public CreateOrientedBoundingBox(center?: THREE.Vector3, halfSize?: THREE.Vector3)
    {
        ...
        this.orientedBoundingBox = new OBB();
        const color = new THREE.Color().setColorName("Red");
        const obbGeometry = new THREE.BoxGeometry(this.halfSize.x * 2, this.halfSize.y * 2, this.halfSize.z * 2);
        obbGeometry.userData.obb = new OBB(center, this.halfSize);
    ...
    {
        const vec3pos = new THREE.Vector3(0, 0, 0);
        const vecd3 = new THREE.Vector3(0, -1, 0);
    ...
    public DeleteCollider()
    {
        if (this.boundingBox)
        {
            ...
            // 순정 three r134 의 Box3Helper 에는 dispose() 가 없다.
            // 예전엔 three.js 본체에 dispose() 를 손으로 추가해서 썼지만(→ 버전 고정의 원인),
            // 이제 호출부에서 직접 해제한다. CLAUDE.md §7.1
    ...
    private orientedBoundingBox: OBB = null;
