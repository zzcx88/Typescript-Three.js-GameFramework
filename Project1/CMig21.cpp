...
CMig21::CMig21(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature)
{
	m_bReffernce = true;
	m_ObjManager = GET_MANAGER<ObjectManager>();
	m_pModelInfo = CGameObject::LoadGeometryAndAnimationFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/Mig-21_root.bin", NULL, MODEL_STD);
...
void CMig21::OnPrepareAnimate()
{
	m_pLeftWingEdge = FindFrame("Left_WingEdge");
	m_pRightWingEdge = FindFrame("Right_WingEdge");
}

void CMig21::Animate(float fTimeElapsed)
{
	if (m_pLeftWingEdge == NULL)
	{
		OnPrepareAnimate();
	}
		...
		if (SphereCollider)SphereCollider->m_xmf4x4ToParent = Matrix4x4::Multiply(XMMatrixScaling(10.f, 10.f, 10.f), m_xmf4x4ToParent);
...
void CMig21::CollisionActivate(CGameObject* collideTarget)
{
