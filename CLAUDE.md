...
#### "A 하고 B 하자" — 한 번에 처리하지 않는다

사용자가 **여러 단계를 한 문장으로 묶어** 지시하는 일이 잦다.

> "미사일까지 수정하고 병합을 하자" · "커밋 먼저 하고 최적화 진행하자" · "점검 먼저 하고 진행하자"

이건 **B를 승인한 게 아니라 순서를 알려준 것**이다. A를 끝내고 **거기서 멈춰 보고**한다.
A와 B 사이에는 거의 항상 **사용자만 할 수 있는 확인**이 끼어 있다 — 브라우저 동작 확인, 결과 판단, 방향 재검토.

```
지시: "미사일 수정하고 병합하자"

✗ 수정 → 커밋 → 병합                    ← 사용자가 미사일을 쏴보기도 전에 병합됨
✓ 수정 → 검증 → 보고 → (사용자 테스트) → 커밋 → 병합
```

**판단 기준**: B가 A의 결과에 의존하는가? 의존하면 멈춘다.
커밋·병합·푸시·배포처럼 **되돌리기 비용이 있는 단계는 예외 없이** 앞 단계 확인 후에 한다.

한 번에 진행해도 되는 경우는 **A와 B가 서로 독립적이고 둘 다 검증이 불필요할 때**뿐이다(예: 문서 두 곳 수정).
