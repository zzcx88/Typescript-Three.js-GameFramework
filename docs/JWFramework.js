        ...
        Update() {
            ...
            if (this.orientedBoundingBox) {
                if (this.gameObject.Picked == true)
                    this.obbBoxHelper.scale.set(this.gameObject.PhysicsComponent.GetScale().clone().x, this.gameObject.PhysicsComponent.GetScale().clone().y, this.gameObject.PhysicsComponent.GetScale().clone().z);
                this.obbBoxHelper.rotation.set(this.gameObject.PhysicsComponent.GetRotateEuler().clone().x, this.gameObject.PhysicsComponent.GetRotateEuler().clone().y, this.gameObject.PhysicsComponent.GetRotateEuler().clone().z);
                this.obbBoxHelper.position.set(this.gameObject.PhysicsComponent.GetPosition().clone().x, this.gameObject.PhysicsComponent.GetPosition().clone().y, this.gameObject.PhysicsComponent.GetPosition().clone().z);
                this.orientedBoundingBox.copy(this.obbBoxHelper.geometry.userData.obb);
                this.orientedBoundingBox.applyMatrix4(this.obbBoxHelper.matrixWorld.clone());
            }
            if (this.raycaster) {
    ...
    class EditObject extends JWFramework.GameObject {
        ...
        Animate() {
            ...
            if (this.isTarget == true) {
                this.PhysicsComponent.MoveFoward(280);
            ...
            if (this.Picked == true) {
                ...
                if (JWFramework.InputManager.getInstance().GetKeyState('space', JWFramework.KeyState.KEY_PRESS)) {
                    ...
                    let missileDirection = targetPos.clone().sub(this.PhysicsComponent.GetPosition());
                    let range = missileDirection.length();
                    let guidanceDirection = missileDirection.multiplyScalar((this.currentVelocity.clone().length()) / range).sub(this.currentVelocity).normalize();
                    this.currentVelocity.add(guidanceDirection.multiplyScalar(25 * JWFramework.WorldManager.getInstance().GetDeltaTime()));
                    this.physicsComponent.SetPostionVec3(this.PhysicsComponent.GetPosition().add(this.currentVelocity));
